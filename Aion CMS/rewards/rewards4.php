...
# ----------------------------
#
# This file contains settings for redeeming vote points (category 4 of rewards)
# 
#  */
...
function GetRewardData()
	{
		...
		$RewardInfo = "{";
		$res = mysql_query("SELECT id,realm,name,description,points FROM voterewards4");
		while($Row = mysql_fetch_array($res))
...
function GetRewardsFromDatabase()
	{
		$Con = mysql_connect(Gamedb_host,Gamedb_user,Gamedb_pass,true);
		mysql_select_db(Gamedb,$Con);
		$res = mysql_query("SELECT * FROM voterewards4 WHERE realm='1' ORDER BY id");
		while($Row = mysql_fetch_array($res))
		{
		$item_id = $Row['itemid'];
