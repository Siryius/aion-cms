...
# ----------------------------
#
# This file contains settings for redeeming vote points (category 9 of rewards)
# 
#  */
...
function GetRewardData()
	{
		...
		$RewardInfo = "{";
		$res = mysql_query("SELECT id,realm,name,description,points FROM voterewards9");
		while($Row = mysql_fetch_array($res))
...
function GetRewardsFromDatabase()
	{
		$Con = mysql_connect(Gamedb_host,Gamedb_user,Gamedb_pass,true);
		mysql_select_db(Gamedb,$Con);
		$res = mysql_query("SELECT * FROM voterewards9 WHERE realm='1'"); // MODIFY HERE votereward table from DB
		while($Row = mysql_fetch_array($res))
		{
		$item_id = $Row['itemid'];
...
function GetCategoryTitleFromDatabase()
	{
		$Con = mysql_connect(Gamedb_host,Gamedb_user,Gamedb_pass,true);
		mysql_select_db(Gamedb,$Con);
		$res = mysql_query("SELECT * FROM votemodcat WHERE id='9'"); // MODIFY HERE ID
